...
const argonParser = {
  ...
  attributes: function(attribs, content = false) {
    if (attribs) {
      return attribs.replace(rgx.attributes, function(match, ref, name, value, value2) {
        ...
        if (ref) {
          return ' ' + argonParser.ref(ref, content);
        } else {
  ...
  singleTag: function(str) {
    return str.replace(rgx.singleTag, function(match, tag, attr) {
      attr = argonParser.attributes(attr);
      return '<' + tag + attr + '>';
    });
  }
}
...
function argon(string) {
  string = argonParser.singleWord(string);
  string = argonParser.multiWord(string);
  string = argonParser.singleTag(string);
  return string;
}
